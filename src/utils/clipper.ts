...
export function clipperPath(fabricObjects: FabricObject[], type: number) {
  ...
  const subjPathPoints = getPathPoints(
    subjPath,
    subjPath.scaleX,
    subjPath.scaleY
  );
  const clipPathPoints = getPathPoints(
    clipPath,
    clipPath.scaleX,
    clipPath.scaleY,
    -pathOffsetX + x,
  ...
  const solutionPaths = new ClipperLib.Paths() as ArrayLike<any>;
...
const getPathPoints = (
  ...
  for (let i = 0; i < itemPath.length; i++) {
    ...
    for (let c = 0; c < Raphael.getTotalLength(item.trim() + " Z"); c += 1) {
      const point = Raphael.getPointAtLength(item.trim() + " Z", c);
