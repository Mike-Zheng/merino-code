...
export default () => {
  ...
  const { templates, templateIndex, currentTemplate } =
    storeToRefs(templatesStore);
  // const mainStore = useMainStore()
  // const slidesStore = useSlidesStore()
  const { selectedTemplatesIndex: _selectedTemplatesIndex } =
  ...
  const selectedTemplates = computed(() =>
    templates.value.filter((item, index) =>
      selectedTemplatesIndex.value.includes(index)
    )
  );
  const selectedTemplatesId = computed(() =>
    selectedTemplates.value.map((item) => item.id)
  );
  ...
  const getEmptyTemplate = (): Template => {
    const emptyTemplate: Template = {
      ...
      objects: currentTemplate.value.objects.filter(
