...
export default () => {
  ...
  const addElementsFromData = (elements: CanvasOption[]) => {
    ...
    const currenttemplateElementIdList = currentTemplate.value.objects.map(
      (el) => el.id
    );

    for (const element of elements) {
      const inCurrenttemplate = currenttemplateElementIdList.includes(
        element.id
      );
      ...
      if (element.type === ElementNames.GROUP)
        element.id = groupIdMap[element.id];
    }
