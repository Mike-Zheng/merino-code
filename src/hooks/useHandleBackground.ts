...
export default () => {
  ...
  const setBackgroudImage = async (dataURL: string) => {
    ...
    const workWidth = workSpaceDraw.width ? workSpaceDraw.width : 0,
      workHeight = workSpaceDraw.height ? workSpaceDraw.height : 0;
    const backgroundImage = await Image.fromURL(dataURL);
    let scaleX = 1,
      scaleY = 1;
    if (currentTemplate.value.workSpace.imageSize === "cover") {
      (scaleX =
        workWidth / (backgroundImage.width ? backgroundImage.width : 1)),
        (scaleY =
          workHeight / (backgroundImage.height ? backgroundImage.height : 1));
    }
