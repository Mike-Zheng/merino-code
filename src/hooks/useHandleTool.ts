...
export default () => {
  const alignElement = (command: AlignCommand) => {
    ...
    if (handleElement.type === ElementNames.ACTIVE) {
      ...
      const activeObjectLeft = activeObject.left - activeObject.width / 2,
        activeObjectTop = activeObject.top - activeObject.height / 2;
      switch (command) {
        ...
        case AlignCommand.RIGHT:
          activeObject._objects.forEach((item) =>
            item.set({
              left: activeObjectLeft + activeObject.width - item.width
            })
          );
          break;
        ...
        case AlignCommand.BOTTOM:
          activeObject._objects.forEach((item) =>
            item.set({
              top: activeObjectTop + activeObject.height - item.height
            })
          );
          break;
        case AlignCommand.HORIZONTAL:
          activeObject._objects.forEach((item) =>
            item.set({ left: activeObject.left - item.width / 2 })
          );
          break;
        case AlignCommand.VERTICAL:
          activeObject._objects.forEach((item) =>
            item.set({ top: activeObject.top - item.height / 2 })
          );
          break;
