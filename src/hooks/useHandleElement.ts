...
export default () => {
  ...
  const sortElement = async (eventData: {
    ...
    if (!element) return;
    if (element.group) {
      const elementGroup = queryOption(
        (element.group as GroupElement).id
      ) as Group;
      ...
      const _elements = JSON.parse(
        JSON.stringify(currentTemplate.value.objects)
      );
  ...
  const pasteElement = async () => {
    ...
    const clonedObj = (await clonedObject.value.clone(
      propertiesToInclude
    )) as FabricObject;
  ...
  const deleteElement = (eid: string) => {
    ...
    if (element.group) {
      if ((element.group as GroupElement)._objects.length === 1) {
        ...
        if (
          element.type === ElementNames.TEXTBOX &&
          deleteTextbox(element as TextboxElement)
        )
          return;
  ...
  const intersectElements = (val: number) => {
    ...
    if (
      ...
      activeObjects[0].type === ElementNames.GROUP
    ) {
  ...
  const queryElement = (eid: string): FabricObject | undefined => {
    ...
    const elements = canvas
      .getObjects()
      .filter(
        (item) => !WorkSpaceCommonType.includes((item as FabricObject).id)
      );
    const element = elements.filter(
      (obj) => (obj as FabricObject).id === eid
    )[0] as FabricObject;
    if (!element) {
  ...
  const queryTextboxChecked = (elements: FabricObject[]): boolean => {
    for (let i = 0; i < elements.length; i++) {
      ...
      if (
        element.type === ElementNames.TEXTBOX &&
        (element as TextboxElement).editable
      ) {
        return true;
      }
  ...
  const checkElement = (eid: string) => {
    ...
    const elements = canvas
      .getObjects()
      .filter(
        (item) => !WorkSpaceCommonType.includes((item as CanvasElement).id)
      ) as FabricObject[];
