...
export const handleMessage = async (template: Template) => {
  // const template = JSON.parse(e.data.template) as Template
  const templateWidth = template.width / template.zoom,
    templateHeight = template.height / template.zoom;
  console.log("template:", template);
  const pdfDoc = await PDFDocument.create();
...
const drawItem = async (
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    if (
      item.type.toLowerCase() === ElementNames.TEXT ||
      item.type.toLowerCase() === ElementNames.TEXTBOX
    ) {
      ...
      item.type.toLowerCase() === ElementNames.IMAGE ||
      item.type.toLowerCase() === ElementNames.CROPIMAGE
    ) {
...
const drawText = async (
  ...
  if (WEB_FONTS.filter((ele) => ele.value === item.fontFamily)[0]) {
    pdfFont = await pdfDoc.embedFont(
      `/src/assets/fonts/${item.fontFamily}.ttf`
    );
  } else {
...
const drawImage = async (
  ...
  if (linkType === "jpg") {
    const jpgImage = await pdfDoc.embedJpg(
      await fetch(imageUrl).then((res) => res.arrayBuffer())
    );
    await imageDraw(item, page, jpgImage);
    return;
