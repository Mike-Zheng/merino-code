...
export const useTemplatesStore = defineStore("Templates", {
  ...
  getters: {
    ...
    currentTemplateElement(state) {
      ...
      return currentTemplate.objects.filter(
        (ele) => ele.id === activeObject.id
      )[0];
    }
  },
  ...
  actions: {
    ...
    modifedElement() {
      ...
      this.templates[this.templateIndex].backgroundImage =
        canvasTemplate.backgroundImage;
      addHistorySnapshot();
    },
      ...
      objects.forEach((ele) => {
        ...
        if (ele.type.toLowerCase() === ElementNames.GROUP) {
          this.setObjectFilter(
            (ele as GroupElement).objects as CanvasElement[]
          );
        }
      });
    },
    ...
    deleteTemplate(templateId: string | string[]) {
      ...
      this.templates = this.templates.filter(
        (item) => !templateIds.includes(item.id)
      );
      addHistorySnapshot();
    },
    ...
    addElement(element: FabricObject | FabricObject[]) {
      ...
      const currentTemplateElements =
        this.templates[this.templateIndex].objects;
      const newElements = [...currentTemplateElements, ...elements];
      this.templates[this.templateIndex].objects =
        newElements as FabricObject[];
      addHistorySnapshot();
    },
    ...
    deleteElement(elementId: string | string[]) {
      ...
      const currentTemplateElements =
        this.templates[this.templateIndex].objects;
      const newElements = currentTemplateElements.filter(
        (item) => !elementIds.includes(item.id)
      );
