...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { picFile, isErasing, radius, hardness, brushSize, hardnessText } =
      useMatting();
    const {
      width,
      height,
    ...
    const downloadFileName = computed(() =>
      picFile.value ? `matting_${picFile.value.name}` : "null"
    );
    ...
    const onFileChange = (ev: Event) => {
      ...
      if (
        files &&
        files[0] &&
        /.+\.(jpg|jpeg|png|gif|webp)/.test(files[0].name)
      ) {
        picFile.value = files[0];
      } else {
        alert(
          "未选择图片或图片格式不正确(只支持jpg、png、gif、webp), 请重新选择"
        );
      }
    };
    ...
    const initLoadImages = (source: string, result: string) => {
      nextTick(() => {
        fetch(source)
          ...
          .then((blob) => {
            picFile.value = new File([blob], `image_${Math.random()}.jpg`, {
              type: "image/jpeg"
            });
          })
          .catch((error) => {
    ...
    function handlechangeRadius(e: any) {
      ...
      if (e.code === "BracketLeft") {
        radius.value > RADIUS_SLIDER_MIN + RADIUS_SLIDER_STEP &&
          (radius.value -= RADIUS_SLIDER_STEP);
      } else if (e.code === "BracketRight") {
        radius.value < RADIUS_SLIDER_MAX - RADIUS_SLIDER_STEP &&
          (radius.value += RADIUS_SLIDER_STEP);
      }
    }
    ...
    function onDownloadResult() {
      if (mattingSources.value && !generating.value) {
        ...
        const url = generateResultImageURL(
          mattingSources.value.orig,
          outputHiddenCtx.value
        );
