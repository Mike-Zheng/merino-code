import {
  DRAW_INTERPOLATION_RADIUS_THRESHOLD,
  DRAW_INTERPOLATION_STEP_BASE
} from "../constants";
...
/** 是否需要插值渲染 */
export function needDrawInterpolation(
  maxMovement: number,
  radius: number
): boolean {
  return (
    radius > DRAW_INTERPOLATION_RADIUS_THRESHOLD &&
    maxMovement > radius / DRAW_INTERPOLATION_STEP_BASE
  );
}

/** 计算插值的步长 */
export function computeInterpolationStep(
  interpolationConfig: RenderInterpolationConfig
): InterpolationStep {
  const { drawingConfig, unsignedMovementX, unsignedMovementY, maxMovement } =
    interpolationConfig;
  ...
  const stepX = computePivotStep(
    movementXIsMaximum,
    rawStepX,
    unsignedMovementX,
    unsignedMovementY
  );
  const stepY = computePivotStep(
    !movementXIsMaximum,
    rawStepY,
    unsignedMovementY,
    unsignedMovementX
...
function computePivotStep(
  ...
  return isMaxMovement
    ? rawStepOfPivot
    : (unsignedPivotMovement / unsignedCrossedMovement) * rawStepOfPivot;
}

/** 判断是否需要绘制插值圆点 */
export function needDrawInterpolationPoint(
  movement: number,
  moved: number,
  step: number
) {
