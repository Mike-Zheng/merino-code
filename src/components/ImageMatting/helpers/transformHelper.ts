...
/** 生成表示绘制范围各个值偏移量的对象 */
export function generateRangeOffset(
...
/** 根据当前鼠标位置更新绘制范围 */
export function updateRangeByMovements(
  ev: MouseEvent,
...
/** 变换(平移、缩放)时重绘画板中图像 */
export function redrawMattingBoardsWhileScaling(
  ev: WheelEvent,
  scalingConfig: InitMattingTransformConfig
) {
  const {
    transformConfig,
...
/** 鼠标滚轮滚动缩放时更新变换参数 */
function updateTransformConfigWhileScaling(
  ev: WheelEvent,
  transformConfig: TransformConfig
) {
  const { deltaY, pageX, pageY, target } = ev;
  const { positionRange, scaleRatio } = transformConfig;
  const { left, top } = computeHelpers.computeBoardRect(
  ...
  const targetScaleRatio = transformHelpers.computeNewScaleRatio(
    scaleRatio,
    deltaRatio
  );
  transformConfig.scaleRatio =
    transformHelpers.computeClampedTargetScaleRatio(targetScaleRatio);
  // 不能直接使用deltaRatio，scaleRatio接近最大/最小值时，二者就不相等了。
  const rangeScaleRatio = computeRangeScaleRatio(
    transformConfig.scaleRatio,
    scaleRatio
  );
  transformConfig.positionRange = transformHelpers.computeNewPositionRange(
    positionRange,
    { x, y },
    rangeScaleRatio
  );
...
function computeDeltaRatio(deltaY: number) {
  const scaleCoefficient = transformHelpers.isZoomOut(deltaY)
    ? ZOOM_OUT_COEFFICIENT
    : ZOOM_IN_COEFFICIENT;
...
function computeClampedTargetScaleRatio(scaleRatio: number): number {
  return scaleRatio < MIN_SCALE_RATIO
    ? MIN_SCALE_RATIO
...
/** 计算缩放后x/y轴方向的新的绘制范围值 */
function computeNewSingleRange(
  singleRange: number,
  pivot: number,
  deltaRatio: number
): number {
