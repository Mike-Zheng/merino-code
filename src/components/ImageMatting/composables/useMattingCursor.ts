...
export class MattingCursor {
  ...
  inputCanvas = computed(
    () =>
      (this.inputCtx.value as CanvasRenderingContext2D).canvas as HTMLElement
  );
  pointInnerColor = computed(() =>
    this.isErasing.value ? ERASE_POINT_INNER_COLOR : REPAIR_POINT_INNER_COLOR
  );
  pointOuterColor = computed(() =>
    this.isErasing.value ? ERASE_POINT_OUTER_COLOR : REPAIR_POINT_OUTER_COLOR
  );

  constructor(
    private inputCtx: Ref<CanvasRenderingContext2D | null>,
    private isErasing: Ref<boolean>
  ) {
...
export default function useMattingCursor(config: UseCursorConfig) {
  ...
  watch(
    [isDragging],
    mattingCursor.changeOutputCursorByDrag.bind(mattingCursor)
  );
