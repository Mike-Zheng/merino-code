...
import {
  initDragListener,
  initScaleListener
} from "../helpers/initTransformListener";
...
export function useInitDrawingListeners(
  props: MattingProps,
...
export function useInitTransformListener(config: UseInitListenersConfig) {
  ...
  const { inputCtx, inputHiddenCtx, outputCtx, outputHiddenCtx } =
    boardContexts;
  watch(
  ...
  onBeforeUnmount(() => {
    if (initialized.value) {
      listenerManager.removeMouseListeners(
        (outputCtx.value as CanvasRenderingContext2D).canvas
      );
