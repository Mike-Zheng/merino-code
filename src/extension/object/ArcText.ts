...
export class ArcText extends OriginIText {
  ...
  renderCharCallback(
    ...
    for (let index = charIndex; index <= endCharIndex; index++) {
      ...
      if (tr.char) {
        const angle =
          this.curvature > 0 ? -tr.charAngle : -tr.charAngle - Math.PI;
  ...
  override getSelectionStartFromPointer(e: TPointerEvent): number {
    ...
    let relX =
        mouseOffset.x + (-this.width / 2 + this._contentOffsetX) * this.scaleX,
      relY =
        mouseOffset.y +
        (-this.height / 2 - this._curvingCenter.y + this._contentOffsetY) *
          this.scaleY,
    ...
    const specials = this._specialArray && this._specialArray[selectedLine];
    let specialsLen = 0;
    let diff = Infinity,
      diff2,
      j;
    for (j = 0; j < this._charTransformations[selectedLine].length; j++) {
      ...
      diff2 =
        Math.abs(this._charTransformations[selectedLine][j].leftAngle - angle) %
        (Math.PI * 2);
      if (diff < diff2) {
        const result = charIndex + j - 1 - specialsLen;
        specialsLen = 0;
        return result;
      }
      diff = diff2;
      ...
      for (j = 0; j < this._textLines[i].length; j++) {
        ...
        if (
          ...
          if (lastDecoration && lastFill) {
            // @ts-ignore
            const offset = (this.offsets[type] as any) * size + dy;
            this._drawTextLinesDecorationSector(
              ctx,
              lastFill,
              offset,
              i,
  ...
  override initDimensions() {
    ...
    for (let i = 0; i < this.__charBounds.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (curvature > 0) {
          ...
          lineRadius =
            deltaY + rowOffset + heightOfLine - heightOfLine / this.lineHeight;
          ...
          charAngle =
            -(currentLeft + bounds.left + bounds.width / 2) / midRadius;
        } else {
          bottomRadius = deltaY + rowOffset;
          topRadius = deltaY + rowOffset - heightOfLine;
          charRadius = deltaY + rowOffset - charOffset;
          ...
          const midRadius = (bottomRadius * 2 + topRadius * 3) / 5;
          leftAngle = Math.PI + (currentLeft + bounds.left) / midRadius;
          rightAngle =
            Math.PI + (currentLeft + bounds.left + bounds.width) / midRadius;
          charAngle =
            Math.PI +
            (currentLeft + bounds.left + bounds.width / 2) / midRadius;
        }
        ...
        const ct = {
          contour: bounds.contour && {
            ...
            y:
              this._getBaseLine(decl.fontSize) +
              bounds.contour.y * decl.fontSize!
          },
        ...
        if (ct.char?.trim() && bounds.contour) {
          const cos = util.cos(-charAngle),
            sin = util.sin(-charAngle);
          const rotateMatrix = [cos, sin, -sin, cos, 0, 0];
          const matrix = util.multiplyTransformMatrices(
            [1, 0, 0, 1, ct.lc.x, ct.lc.y],
            rotateMatrix as TMat2D
          );
          const y = ct.contour.y;
          if (curvature > 0) {
            const x = ct.contour.x - this.__charBounds[i][j].width / 2;
            ct.contour.br = util.transformPoint(
              { x: x + ct.contour.w, y: -y },
              matrix
            );
            ct.contour.bl = util.transformPoint({ x: x, y: -y }, matrix);
            ct.contour.tl = util.transformPoint(
              { x: x, y: -y - ct.contour.h },
              matrix
            );
            ct.contour.tr = util.transformPoint(
    ...
    this.width = Math.max(
      textWidth + leftOverflow + rightOverflow,
      this.MIN_TEXT_WIDTH
    );
    ...
    const _translateX =
      this.originX === "left" ? leftOverflow : this._contentOffsetX;
  ...
  interateTextChunks(lineIndex: number, foo: Function, iteratorFn?: Function) {
    let actualStyle: any,
      nextStyle,
      firstChar = 0;
    const specs = this._specialArray;
    ...
    for (let i = 0, len = line.length - 1; i <= len; i++) {
      ...
      if (!timeToRender) {
        ...
        timeToRender =
          (specs &&
            specs[lineIndex] &&
            specs[lineIndex][i] !== specs[lineIndex][i + 1]) ||
  ...
  _drawTextLinesDecorationSector(
    ctx: CanvasRenderingContext2D,
    currentColor: string,
    offset: number,
    line: number,
    ...
    const startChar = this._charTransformations[line][charStart];
    const endChar = this._charTransformations[line][charEnd - 1];
    ctx.beginPath();
    if (this.curvature < 0) {
      ctx.arc(
  ...
  _contextSelectBackgroundSector(
    ctx: CanvasRenderingContext2D,
    line: number,
    charStart: number,
    charEnd: number,
    ...
    ctx.beginPath();
    const startChar = this._charTransformations[line][charStart];
    const endChar = this._charTransformations[line][charEnd];
    ctx.moveTo(startChar.tl.x, startChar.tl.y);
    const radius = fullLineRadius
      ? startChar.bottomRadius
      : startChar.lineRadius;
  ...
  renderSelection(ctx: CanvasRenderingContext2D, boundaries: CursorBoundaries) {
    ...
    for (let i = startLine; i <= endLine; i++) {
      const charStart = i === startLine ? startChar : 0,
        charEnd =
          i >= startLine && i < endLine ? this._textLines[i].length : endChar;
      this._contextSelectBackgroundSector(
        ctx,
        i,
        charStart,
        charEnd - 1,
  ...
  override _measureLine(lineIndex: number) {
    ...
    for (charIndex = 0; charIndex < line.length; charIndex++) {
      ...
      const style = this.getCompleteStyleDeclaration(
        lineIndex,
        charIndex
      ) as any;
      ...
      if (this.useRenderBoundingBoxes && graphemeInfo.contour) {
        ...
        contourY =
          this._getBaseLine(style.fontSize) +
          graphemeInfo.contour.y * style.fontSize;
        renderedLeft = Math.max(renderedLeft, -(graphemeInfo.left + contourX));
        renderedWidth = Math.max(
          renderedWidth,
          contourW + contourX + graphemeInfo.left
        );
  ...
  renderCursor(ctx: CanvasRenderingContext2D, boundaries: any) {
    ...
    const tr =
      this._charTransformations[cursorLocation.lineIndex][
  ...
  _splitText() {
    ...
    const newLines = this._splitTextIntoLines(text);
  ...
  _renderStr(
    ...
    if (this.renderCharCallback) {
      ...
      const text = this._textLines[lineIndex]
        .slice(charIndex, endCharIndex + 1)
        .join("");
  ...
  _setSVGTextLineText(
    ...
    for (let i = 0, len = line.length - 1; i <= len; i++) {
      ...
      const angle =
        this.curvature > 0 ? -charBox.charAngle : -charBox.charAngle - Math.PI;
      if (boxWidth === 0) {
