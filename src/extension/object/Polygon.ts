...
export class Polygon extends OriginPolygon {
      ...
      const objects = group.getObjects().filter((obj) => {
        if (this.ignoreObjTypes.length) {
          return !this.ignoreObjTypes.some(
            (item) => obj.get(item.key) === item.value
          );
        }
    ...
    for (let i = canvasObjects.length; i--; ) {
      ...
      Keys(objCoordsByMovingDistance).forEach((activeObjPoint) => {
        ...
        Keys(newCoords).forEach((objPoint) => {
          if (
            this.isInRange(
              objCoordsByMovingDistance[activeObjPoint].y,
      ...
      Keys(objCoordsByMovingDistance).forEach((activeObjPoint) => {
        ...
        Keys(newCoords).forEach((objPoint) => {
          if (
            this.isInRange(
              objCoordsByMovingDistance[activeObjPoint].x,
              objCoords[objPoint].x
            )
          ) {
  ...
  private getObjDraggingObjCoords(): ACoordsAppendCenter {
    ...
    const newCoords = Keys(coords).map((key) =>
      coords[key].subtract(centerPoint)
    );
    return {
      tl: newCoords[0],
      tr: newCoords[1],
  ...
  renderEndStyle(ctx: CanvasRenderingContext2D) {
    ...
    if (this.endStyle === ElementNames.ARROW) {
      ctx.save();
      ctx.translate(
        (lastPoint.x - this.points[0].x) / 2,
        (lastPoint.y - this.points[0].y) / 2
      );
      ctx.rotate(angle);
      ctx.beginPath();
      ctx.moveTo(this.pointSize, 0);
