...
export class CurvedText extends IText {
  ...
  _render() {
    ...
    if (this.letters && this.letters._objects.length) {
      ...
      for (let i = 0; i < this.text.length; i++) {
        for (const key in this._dimensionAffectingProps) {
        ...
        if (this.effect === "curved") {
          thisLetterAngle =
            (this.letters._objects[i].width + space) / this.radius / halfPI;
          currentAngle =
            multiplier * (multiplier * currentAngle + lastLetterAngle);
          ...
          currentAngle =
            multiplier * (multiplier * currentAngle + fixedLetterAngle);
          angleRadians = currentAngle * halfPI;
          this.letters._objects[i].set({
            angle: currentAngle,
            padding: 0,
          ...
          const small = this.largeFont;
          const large = this.smallFont;
          const difference = large - small;
          const center = Math.ceil(this.text.length / 2);
          const step = difference / this.text.length;
          const newfont = small + i * step;
          ...
          this.letters._objects[i].top =
            -1 * this.letters._objects[i].get("fontSize") +
            i / this.text.length;
        }
      }
  ...
  _getBounds(aX: number[], aY: number[], onlyWidthHeight?: number) {
    ...
    if (!onlyWidthHeight) {
      ...
      this.letters.setPositionByOrigin(
        { x: left, y: top } as Point,
        "left",
        "top"
      );
    }
  ...
  _updateObjectsCoords(center?: Point) {
    const centerPoint = center || this.letters.getCenterPoint();
  ...
  toSVG(reviver: TSVGReviver | undefined): string {
    const markup = ["<g ", this.getSvgTransform(), ">"];
    if (this.letters) {
      for (let i = 0, len = this.letters.size(); i < len; i++) {
