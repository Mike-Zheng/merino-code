...
export function canvasMouseMove({ e }: TPointerEventInfo<MouseEvent>) {
  ...
  if (
    !activeObject ||
    !activeObject.canvas?.__targetlessCanvasDrag ||
    e.type !== "mousemove" ||
    !activeObject
  ) {
    return;
  ...
  const totalM = util.invertTransform(
    util.multiplyTransformMatrices(canvasM, objM)
  );
  totalM[4] = 0;
  totalM[5] = 0;
  ...
  const cropping: any = {
    ...
    _drawDarkLayer(ctx: CanvasRenderingContext2D) {
      ...
      ctx.fillRect(
        0,
        0,
        this.canvas.lowerCanvasEl.width,
        this.canvas.lowerCanvasEl.height
      );
      ctx.restore();
    },
    ...
    _drawCroppingPath(ctx: CanvasRenderingContext2D) {
      ...
      ctx.globalAlpha = 1;
      ctx.strokeStyle = this.cropLinesColor;
      this.cropPath = CLIPPATHS[this.cropKey as ClipPathType].createPath(
        this.width,
        this.height
      );
