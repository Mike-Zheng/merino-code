    ...
    >
      <el-checkbox-group v-model="showWorkLines" @change="changeWorkLines">
        <el-tooltip
          ...
          :hide-after="0"
          :content="
            showClip ? t('message.hideBloodsLine') : t('message.showBloodsLine')
          "
        >
          <el-checkbox-button
            type="primary"
        ...
        <el-tooltip
          ...
          :hide-after="0"
          :content="
            showSafe ? t('message.showSafeLine') : t('message.hideSafeLine')
          "
        >
          <el-checkbox-button
            type="primary"
...
const changeWorkLines = () => {
  ...
  const WorkSpaceClip = canvas
    .getObjects()
    .filter((ele) => (ele as CanvasElement).id === WorkSpaceClipType);
  const WorkSpaceSafe = canvas
    .getObjects()
    .filter((ele) => (ele as CanvasElement).id === WorkSpaceSafeType);
