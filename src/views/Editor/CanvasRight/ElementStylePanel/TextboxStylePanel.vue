<template>
  <div class="text-style-panel">
          ...
          <div @click.stop class="tooltip-popover">
            <el-popover trigger="click" placement="bottom" :width="265">
              ...
              <ColorPicker
                ...
                @update:modelValue="
                  (color: string) => updateBackgroundColor(color)
                "
              />
            </el-popover>
          </div>
    ...
    <el-row
      class="mt-10"
      v-show="handleElement.type.toLowerCase() === ElementNames.ARCTEXT"
    >
      <el-col :span="4" class="flex-align">
        <el-radio-group
          class="full-ratio"
          v-model="handleElement.showCurvature"
...
const handleElement = computed(
  () => canvasObject.value as TextboxElement | ArcText
);
...
const hasFontWeight = computed(
  () => handleElement.value.fontWeight !== "normal"
);
...
const hasPatterns = computed(() =>
  (handleElement.value as TextboxElement).fillType === 1 ? true : false
);
    ...
    if (handleElement.value.fontWeight === fontBold) {
      ...
      for (const i in elementStyle) {
        for (const j in elementStyle[i]) {
          (elementStyle[i][j] as TextboxElement).set({
...
const handleElementItalic = () => {
  const fontStyle =
    handleElement.value.fontStyle === "italic" ? "normal" : "italic";
...
const handleElementLinethrough = () => {
  if (handleElement.value.isEditing) {
    handleElement.value.setSelectionStyles({
      linethrough: !handleElement.value.linethrough
    });
  } else {
...
const handleElementUnderline = () => {
  if (handleElement.value.isEditing) {
    handleElement.value.setSelectionStyles({
      underline: !handleElement.value.underline
    });
  } else {
...
const handleElementCharSpacing = (mode: "+" | "-") => {
  ...
  const charSpacing =
    mode === "+" ? handleCharSpacing + 10 : handleCharSpacing - 10;
...
const handleElementArrange = (status: boolean) => {
  ...
  const options = handleElement.value.toObject(
...
const handleElementCurve = async () => {
  ...
  const path = fontElement.getPath(
    handleElement.value.text,
    0,
    0,
    handleElement.value.fontSize
  );
  createPathElement(
    path.toPathData(2),
