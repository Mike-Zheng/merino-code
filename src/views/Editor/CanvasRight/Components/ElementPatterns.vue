...
const updatePatternElement = async () => {
  const imageURL = PatternImages.filter(
    (item) => item.name === sourcePattern.value
  )[0].url;
  if (!imageURL) return;
  ...
  handleElement.value.fillRepeat = repeatPattern.value
    ? repeatPattern.value
    : "repeat";
