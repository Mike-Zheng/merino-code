...
const viewportRatio = computed(
  () => props.template.height / props.template.width
);
...
onMounted(() => {
  thumbCanvas.value = new StaticCanvas(thumbnailTemplate.value, {
    ...
    backgroundColor:
      props.template.workSpace.fillType === 0
        ? (props.template.workSpace.fill as string)
        : "#fff"
  });
  setThumbnailElement();
});
...
const setThumbnailElement = async () => {
  ...
  const thumbWorkSpaceDraw = thumbCanvas.value
    .getObjects()
  ...
  const objects = thumbCanvas.value
    .getObjects()
